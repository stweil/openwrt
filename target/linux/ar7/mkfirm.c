...
static void makecrc(void)
{
    ...
    unsigned i;           /* counter for all possible eight bit values */
    unsigned k;           /* byte being shifted into crc apparatus */

    /* terms of polynomial defining this crc (except x^32): */
    static const unsigned p[] = {0,1,2,4,5,7,8,10,11,12,16,22,23,26};
...
static void usage(void)
{
    ...
    fprintf(stderr, "  or:  %s [-o <outfile>] -m <magic> <zipfile>\n", program_name);
    fprintf(stderr, "  or:  %s [-o <outfile>] -m <magic> <pfsfile> <sohofile>\n", program_name);
